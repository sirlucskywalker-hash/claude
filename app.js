...
function exerciseThumbArt(name){
  const n=(name||'').toLowerCase(),incline=n.includes('incline'),press=n.includes('press');
  if(press)return '<svg viewBox="0 0 120 72" aria-hidden="true"><defs><linearGradient id="tg" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#1a2e49"/><stop offset="1" stop-color="#09111e"/></linearGradient></defs><rect width="120" height="72" rx="10" fill="url(#tg)"/><path d="'+(incline?'M28 54L68 31M29 55h54':'M26 53h61')+'" stroke="#6b829f" stroke-width="4" stroke-linecap="round"/><circle cx="58" cy="'+(incline?'27':'31')+'" r="7" fill="#a9b8c9"/><path d="'+(incline?'M56 34l-13 10 19 6 12-13':'M55 38l-15 7 17 7 18-9')+'" fill="none" stroke="#9fb2c8" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/><path d="'+(incline?'M46 39l-8-13M69 39l10-13':'M43 43l-5-15M72 41l7-14')+'" stroke="#b8c6d7" stroke-width="4" stroke-linecap="round"/><circle cx="'+(incline?'36':'37')+'" cy="'+(incline?'24':'27')+'" r="5" fill="#4d6b8e"/><circle cx="'+(incline?'81':'80')+'" cy="'+(incline?'24':'26')+'" r="5" fill="#4d6b8e"/></svg>';
  return '<svg viewBox="0 0 120 72" aria-hidden="true"><rect width="120" height="72" rx="10" fill="#0a1422"/><path d="M27 38h66M35 27v22m50-22v22M22 31v14m76-14v14" stroke="#9eb5d2" stroke-width="5" stroke-linecap="round"/></svg>';
}
...
function dailyScoreForDate(date){
  const x=state.logs.find(v=>v.date===date);if(!x)return 0;let pts=0,total=0;
...
function dailyScore(){return dailyScoreForDate(today())}
function saveLog(){
...
function renderDashboard(){
  ...
  const metric=state.profile.units==='metric'; el('dashCalories').textContent=state.macro?state.macro.calories:'—';el('dashWeight').textContent=latest?(metric?(latest.weight/2.20462).toFixed(1)+' kg':latest.weight.toFixed(1)+' lb'):state.profile.weight?(metric?(state.profile.weight/2.20462).toFixed(1)+' kg':state.profile.weight.toFixed(1)+' lb'):'—';el('dashAdherence').textContent=t&&t.adh?t.adh.toFixed(0)+'%':'—';if(el('dashStreak'))el('dashStreak').textContent=logStreak()+'d';if(el('dailyScore')){const ds=dailyScore(),ring=el('dailyScore').parentElement;el('dailyScore').textContent=ds||'—';ring.style.setProperty('--score',(ds||0)+'%');const prior=dailyScoreForDate(dateDaysAgo(7)),deltaEl=el('dailyScoreDelta');if(deltaEl){if(ds&&prior){const change=Math.round((ds-prior)/Math.max(prior,1)*100);deltaEl.innerHTML='<strong>'+(change>=0?'↑ ':'↓ ')+Math.abs(change)+'%</strong><small>vs. last week</small>'}else deltaEl.innerHTML='<strong>Live</strong><small>this week</small>'}};if(el('timeGreeting')){const h=new Date().getHours();el('timeGreeting').textContent=h<12?'GOOD MORNING':h<17?'GOOD AFTERNOON':'GOOD EVENING'} el('homeCoach').textContent=adaptive();renderGettingStarted();renderToday();renderTodayMetricsSnapshot();renderAdjustment();renderHomeInsight();draw('weightChart','weight','Weight');draw('waistChart','waist','Waist');
}
